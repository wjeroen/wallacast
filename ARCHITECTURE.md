...
- **`components/AudioPlayer.tsx`**: Manages audio playback state (HTMLAudioElement, position saving, speed, sleep timer). Renders either the compact MiniPlayer (above the bottom tab bar) or the FullscreenPlayer overlay. The fullscreen X (top left, faint) minimizes to the mini player whenever any audio exists (original or summary), and the mini player's X (left side, with the play button on the right, Spotify style) is the real close. Items WITHOUT audio render fullscreen only (the mini player is playback chrome, so it never shows for them, and there the fullscreen X closes for real). Handles the iOS headphone-disconnect guard, play/pause icon sync, and podcast audio proxying through the backend.
