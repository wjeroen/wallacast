...
- **`services/llm-alignment.ts`**: LLM-based content-to-transcript alignment for read-along tab (replaces Needleman-Wunsch approach). Quality retry: each pass is scored by the fraction of element timestamps that strictly advance; below 70% (and above 1500 chars of content) the failed batches are re-run ONCE and the best-scoring attempt is kept (a retry can never make the stored alignment worse)
