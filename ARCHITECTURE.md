...
- **`components/FullscreenPlayer.tsx`**: The expanded fullscreen overlay. Contains all tab rendering:
  ...
  - **History tab** (articles/texts, only once at least one prior snapshot exists): lists version-history snapshots (saved before each edit/refetch/restore, never audio) with View and Restore. Restore snapshots the current state first (so it's undoable) and also rolls back title/author/date (COALESCE keeps current values where an old snapshot predates migration 024).
