...
- **`components/SettingsPage.tsx`**: User settings management UI
  ...
  - **API keys** section: DeepInfra, OpenAI, **OpenRouter**, **Anthropic**, Gemini. Each key has a two-line description — line 1 lists the jobs it can power (from: narration, read-along, summaries, TTS, transcription, image descriptions), line 2 is a "Get a key" link (OpenRouter also links to its model-pricing **Compare** page).
