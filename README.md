...
- **`routes/users.ts`**: User settings management (requires JWT auth)
  ...
  - `GET /api/users/summary-prompt-defaults` - Built-in default summary prompts (article/comment/podcast) for pre-filling the Settings "Custom prompts" boxes
...
- **`services/summarizer.ts`**: Twitter-thread style summaries (requires per-user DeepInfra or OpenAI API key)
  ...
  - **Custom prompts (optional)**: the three built-in default prompts (article / comment / podcast) are exported as `*_SUMMARY_TEMPLATE` strings with `{maxTweets}`/`{maxWords}` placeholders. `resolveSummaryPrompt()` uses a per-user override (`summary_article_prompt` / `summary_comment_prompt` / `summary_podcast_prompt`) when set, else the default. A custom prompt applies to **all** length tiers (the `maxTweets<=1` single-paragraph wording is only used by the defaults). `fillTemplate()` substitutes the placeholders. Defaults are served to the Settings UI via `GET /api/users/summary-prompt-defaults` so the boxes pre-fill with the real current default. **Known gotcha for newsletters/digests**: the default article prompt's "develop a single central thesis / single line of reasoning" framing pushes the model to summarize only the lead story of a multi-section newsletter. (The old "...rather than trying to cover them all" clause on the roundup line was removed 2026-06-27.) Use a custom article prompt to make it cover every section. **Note: there are 6 prompt strings, not 3** — each of the article/comment/podcast functions has a single-paragraph branch (`maxTweets <= 1`, inline strings) and a multi-paragraph branch (the exported `*_SUMMARY_TEMPLATE`). The custom-prompt boxes pre-fill from the multi-paragraph templates, and a saved override applies to **all** tier sizes (so it replaces the single-paragraph default too); the single-paragraph defaults themselves are not separately editable.
  - User settings: `auto_generate_summary` (auto-create on add), `summarize_comments` (default on), `summary_tiers` (editable JSON list of `{ maxChars, maxTweets }`), `summary_article_prompt` / `summary_comment_prompt` / `summary_podcast_prompt` (optional custom prompts; blank = built-in default).
...
- **`components/SettingsPage.tsx`**: User settings management UI
  ...
  - **Summaries** section has two collapsibles: "Length settings" (words-per-paragraph + the length-tier editor) and "Custom prompts (advanced)" — three editable textareas (article / comment / podcast) pre-filled with the built-in defaults fetched from `GET /api/users/summary-prompt-defaults`. Edit one to override that summary's system prompt; a "(customized)" tag and an enabled "Reset to default" button show when it differs. On save, a box left identical to the default is stored empty (= keep the built-in default, so future default tweaks still flow through); only genuine edits are persisted to `summary_*_prompt`. Placeholders `{maxTweets}`/`{maxWords}` are substituted server-side.
...
- Summary-related keys: `auto_generate_summary` ('true'/'false'), `summarize_comments` ('true'/'false', default on), `summary_tiers` (JSON list of `{ maxChars, maxTweets }`; the unbounded tier stores `maxChars: null` = Infinity), `summary_max_words` (max words per paragraph/"tweet"; default 40), `summary_article_prompt` / `summary_comment_prompt` / `summary_podcast_prompt` (optional custom system prompts; blank = built-in default; support `{maxTweets}`/`{maxWords}` placeholders), `library_show_summary` ('true'/'false') — when on, library cards show the article `summary` instead of the description (falls back to the description when no summary exists; the list endpoint now also returns `summary`)
