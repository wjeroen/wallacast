...
- **Read-only API access**: Settings → "Read-only API tokens" creates a `wcr_...` token for outside tools such as an Obsidian import script. A token can list the library (`GET /api/content/index`, a lean one-row-per-item index) and fetch the exact "Copy content" Markdown of an item (`GET /api/content/markdown?url=...` or `GET /api/content/:id/markdown`), and nothing else: every other route answers 403, so a leaked token cannot change anything or read anything beyond that. The URL lookup accepts a second address for the same article (`?url=<source>&url=<alt-source>`), which matters for crossposts and for articles saved through an archive mirror
