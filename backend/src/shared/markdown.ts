...
// The `.js` extensions on these relative imports are for the backend, which runs a
...
// The backend serves "Copy content" over the API by running this very file, copied byte
// for byte into backend/src/shared/ (backend/scripts/test-markdown-export.mts fails when
// the copy drifts). Node has no DOM, so the backend installs jsdom's DOMParser through
// setHtmlParser() before the first conversion. Nothing else differs between the two.
