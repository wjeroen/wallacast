...
// assembled dynamically around it. WARNING: read-along sync depends on these rules, a careless
// override can break highlighting. Never instruct fuzzy/algorithmic matching (see CLAUDE.md).
export const ALIGNMENT_RULES_DEFAULT = `Keep in mind that the transcription is generated by AI, so some words and names won't always be transcribed accurately. You need to find the best matches, not exact matches.
...
export async function generateLLMAlignment(
  ...
  const sharedRules = await resolveCustomPrompt(userId, 'prompt_alignment_rules', ALIGNMENT_RULES_DEFAULT);
