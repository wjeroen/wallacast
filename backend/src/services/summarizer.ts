...
export async function generateSummaryForContent(contentId: number): Promise<void> {
  ...
  try {
    ...
    // 2. Tier lookup → maxTweets for the article
    ...
    console.log(`[Summary] articleChars=${articleChars} -> maxTweetsArticle=${maxTweetsArticle} model=${chat.model}`);

    // 4. Article summary
    const articleResponse = await chat.client.chat.completions.create({
      model: chat.model,
      messages: [
        { role: 'system', content: ARTICLE_SUMMARY_PROMPT(maxTweetsArticle) },
    ...
    if (summarizeComments && Array.isArray(comments) && comments.length > 0) {
      ...
      if (commentsText) {
        ...
        const commentResponse = await chat.client.chat.completions.create({
          model: chat.model,
          messages: [
            { role: 'system', content: COMMENT_SUMMARY_PROMPT(maxTweetsComments) },
            {
              role: 'user',
              content:
