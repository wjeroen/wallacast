...
- [x] **PDF image extraction for TTS narration** (2026-03-16): PDFs now have embedded images extracted via `unpdf`'s `extractImages()` + `sharp` (raw pixels → PNG → base64 data URIs). Images are placed in the HTML after each page's text, so the existing Gemini image description → TTS narration pipeline picks them up automatically. `ImageAltTextService` updated to handle `data:` URIs without network requests. Decorative image filter skips regex patterns for data URIs (base64 can randomly match "header", "banner", etc.). Pages processed in 10-page chunks for memory efficiency.
