...
### Bug Fixes
- [ ] **[P3]** Newsletter/digest summaries only cover the lead story — the default article prompt (`summarizer.ts` `ARTICLE_SUMMARY_TEMPLATE`) tells the model to "develop a single central thesis" and "if it's a roundup, just highlight the most interesting ones rather than cover them all", so multi-section newsletters (e.g. Transformer) get summarized down to only THE BIG STORY. Custom-prompt boxes now exist in Settings to test a fix; decide on a default-prompt rewrite (essay-vs-digest shape detector) and apply it. Also verify the full newsletter text actually lands in `html_content` (extraction could be dropping sections).
...
- [x] **Custom summary prompts in Settings** (2026-06-27): the three built-in summary prompts (article/comment/podcast) can now be overridden per-user. Backend extracts them as `*_SUMMARY_TEMPLATE` strings with `{maxTweets}`/`{maxWords}` placeholders; `resolveSummaryPrompt()` + `fillTemplate()` use a saved override (`summary_article_prompt`/`summary_comment_prompt`/`summary_podcast_prompt`) when present, else the default. New `GET /api/users/summary-prompt-defaults` serves the defaults so the Settings → Summaries → **"Custom prompts (advanced)"** collapsible pre-fills three editable textareas with the real current defaults. A box left identical to the default saves empty (= keep built-in, so future default tweaks still flow through); only genuine edits persist. Built as a tool to test prompt fixes for the **newsletter/digest bug** (the default article prompt's "single thesis" + "roundup → highlight the most interesting" lines make multi-section newsletters summarize only the lead story — default prompt left unchanged for now, pending a chosen fix).
