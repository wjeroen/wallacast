...
- [ ] **[P2]** Library filtration v2: combinable filters beyond the current single status pick (audio / no audio, has summary, has transcript, favorites combinable with active/archived). Design discussion in progress, goals: few taps, stays obvious what is filtered, the toolbar row must not get crowded. Audio-only filtering also cleans the player queue ("Up next" reuses `itemMatchesFilter`). Files: `LibraryTab.tsx`, `contentStore.ts` (filter model + matcher).
