...
- [ ] **[P2]** Enrich Whisper prompt with article headings/dates — current prompt only includes title+author (~94 chars for long articles). Should extract section headings and dates from HTML content and add them to the Whisper prompt so Whisper recognizes them as distinct phrases. File: `backend/src/services/openai-tts.ts` (lines 834-861) and `backend/src/services/whisper-prompt.ts`
- [ ] **[P2]** Fix image description toggle not fully working — when `image_alt_text_enabled` is off, Gemini stops generating new descriptions, but old descriptions from previous runs (stored in `image_alt_text_data` DB column) still get injected into TTS via `injectImageNarrations()` at line 720 of `openai-tts.ts`. The injection step doesn't check the toggle. Also, `injectImageNarrations()` falls back to HTML `alt` attributes (Strategy D) even without Gemini data.
