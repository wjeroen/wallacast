...
### Bug Fixes
- [ ] **[P3]** TTS default silently assumes an OpenAI key: with no voices selected, generation uses `gpt-4o-mini-tts` + `coral`, which fails for DeepInfra-only users. Consider falling back to Kokoro via DeepInfra when no OpenAI key is configured (would match the "DeepInfra for TTS" recommendation in Settings).
