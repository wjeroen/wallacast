...
- [x] **[P1]** Fix LLM alignment catastrophic metadata confusion — LLM matched "Written by MaxDalton" to a comment at 301s instead of the author attribution at 2s, cascading error via monotonicity enforcement to destroy ALL timestamps; fixed with: (1) pre-assign metadata timestamps via direct transcript search, (2) outlier detection before monotonicity enforcement, (3) prompt rewrite with sequential processing instructions and pre-matched anchors, (4) swapped elements/transcript order in prompt (2026-02-16)
