...
### Bug Fixes
- [ ] **[P5]** Legacy dead settings cleanup: `ai_provider` and `openai_model` exist only in the users.ts whitelist, no backend service reads them, yet the Settings form still persists them (`'openai'` / `'gpt-5-nano'`) on every save. Remove from formData + whitelist when convenient. `narration_llm` stays (it's the live legacy fallback).
