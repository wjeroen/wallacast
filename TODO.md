...
- [ ] **[P1]** **Move audio blobs out of Postgres onto a Railway volume (the big RAM win).** Audio (`audio_data` BYTEA) lives inside Postgres, so every read pulls 10-50MB into Postgres's RAM cache, where it lingers for days (gold "Postgres" line in the memory graph held 1-3GB for days). Railway bills RAM at ~$10/GB/month vs. volume disk at ~$0.15-0.25/GB/month (~40-66x cheaper). Plan: mount a Railway volume on the backend (e.g. `/data/audio`), store new audio as files + keep only a path in the DB, read/serve from disk, one-time migrate existing blobs, then clear `audio_data`. Backward-compatible/gated rollout. PREREQUISITE: user creates the volume + env var in the Railway dashboard. (See migration plan discussed 2026-06-12.)
- [ ] **[P3]** `GET /api/content` (the main library list) ships `transcript_words` (the 9k-word timestamp JSONB) for EVERY item on every library load — the card badge/"Generate transcript" check only needs to know whether a transcript exists, not all the words. Replace with a boolean (e.g. `transcript_words IS NOT NULL AS has_transcript`) + update the frontend's two consumers. Flagged 2026-06-12, needs user go-ahead.
- [ ] **[P4]** Dead code: `contentAPI.getEpisodes` (frontend `api.ts`) and its backend endpoint `GET /podcasts/:id/episodes` have no callers. Remove both? Flagged 2026-06-12, needs user go-ahead.
- [ ] **[P2]** Verify `POSTGRES_CONFIG=low` actually does anything. Could NOT find any evidence Railway's Postgres template supports this env var (official docs change config via `ALTER SYSTEM SET shared_buffers=...`, not an env var). Likely a no-op. If we want to cap Postgres RAM directly, use `ALTER SYSTEM SET shared_buffers`/`work_mem`/`effective_cache_size` (needs a DB restart) — but the volume migration above is the real fix.
- [ ] **[P3]** Audio finalize holds 2-3 copies of the MP3 in backend RAM (`openai-tts.ts`: `finalBuffer` + the pg driver's copy for the INSERT + any stale blob). Matches the teal "backend" ~1.8GB spikes during generation. Streaming the file to storage would fix it (folds into the volume migration).
