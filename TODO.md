...
- [ ] **[P4]** Strip MathJax `<style>` from EA Forum/LessWrong (GraphQL path) — math articles embed a huge MathJax CommonHTML stylesheet (`.mjx-chtml {...}` wall) right next to the equation. The standard scraper strips `<style>` globally (`article-fetcher.ts` ~603-607), but the **GraphQL path** for EA Forum/LessWrong returns pre-rendered HTML and skips that, so the stylesheet survives — it then dumps into the Markdown editor and gets applied to the page on display. Fix: strip `<style>`/`<script>` in the GraphQL/EA-LW branch too (and as a `remove()` in the turndown converter). Separate/bigger: math renders as plain text (`xa` instead of `xᵃ`) — proper math rendering ($…$/KaTeX) is its own task.
...
- [ ] **[P3]** Read-along: lists and footnote sections are extracted as ONE element (one timestamp), so the whole list/footnote block highlights at once — it sucks. Fix is finer element extraction in `llm-alignment.ts` `extractContentElements()` (split `<ul>`/`<ol>` into per-`<li>` elements; treat the footnote section similarly). NOTE: this is NOT fuzzy matching — it's just splitting elements before the LLM aligns them, so it's allowed. Previously deemed hard; worth revisiting. (User-flagged 2026-06-14.)
