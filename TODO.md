...
- [ ] **[P2]** Remaining `SELECT *` on content_items (loads transcript/transcript_words needlessly): `routes/podcasts.ts:185` (`GET /:id/episodes` — returns full rows incl. transcripts for ALL episodes; needs frontend-consumer audit before narrowing) and `services/wallabag-sync.ts:416`. Replace with explicit column lists like the article list endpoint does.
