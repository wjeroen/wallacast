...
- [ ] **[P1]** Offline support + local caching for snappier experience. Use IndexedDB to cache content locally on-device to make the app faster even when online (no server round-trip to open an item) and fully usable offline.
...
- [x] **[P3]** Conservative light mode improvements. Darken `--bg-app` (#f8fafc → #f1f5f9) for better card contrast, fix play/pause icon staying white on blue button, pastel light-mode overrides for `.generation-status.generating/.error` and `.metadata .badge/.badge.transcript` (2026-04-16)
- [ ] **[P4]** Bulk podcast subscription import (OPML format)
- [x] **[P3]** Allow texts (not articles) to be edited with markdown support, doesn't automatically regenerate audio. Done AND extended to articles: full-fidelity HTML↔Markdown editor (`frontend/src/markdown.ts` using turndown + marked, Obsidian callouts for LLM blocks/tweets, raw-HTML islands for anything unknown). Content/Read-along tabs re-split, Markdown/HTML toggle on Add Text, Copy content now preserves images/links/inline formatting. (2026-06-14)
...
- [x] **[P6]** OpenRouter API compatibility. Done as part of the multi-provider model-selection overhaul (see Completed Recently). OpenRouter + Anthropic + Gemini added as providers, per-job (narration/alignment/summaries/transcription) provider+model+reasoning selection in Settings. (2026-06-21)
- [x] **[P4]** Keep version history of all previous content/comment fetches in case articles get deleted or regeneration went poorly (don't do this with audio, would take up too much space). `content_versions` table snapshots the body (html/content/comments, never audio) before every edit/refetch/restore, viewable + restorable in the fullscreen player's new History tab. Keeps the last 25 per item. (2026-06-14)
- [ ] **[P4]** Implement import functionality (export done via "Download data (zip)" button, import still needed)

### Bug Fixes
- [x] **[P5]** Legacy dead settings cleanup: `ai_provider` and `openai_model` existed only in the users.ts whitelist, no backend service read them, yet the Settings form persisted them on every save. Removed from formData and the whitelist same day (2026-07-02). Old DB rows are harmless orphans. `narration_llm` stays (it's the live legacy fallback).
- [x] **[P3]** TTS default silently assumes an OpenAI key: with no voices selected, generation uses `gpt-4o-mini-tts` + `coral`, which fails for DeepInfra-only users. Fixed 2026-07-02: the default is now key-aware, **Kokoro's Puck (`am_puck`)** whenever a DeepInfra or OpenRouter key exists (Kokoro serving also falls back to OpenRouter when DeepInfra is missing), else OpenAI `gpt-4o-mini-tts` + coral. Also removed the invisible `openai_tts_voice`/`openai_tts_model` form fields that silently re-persisted `coral`/`gpt-4o-mini-tts` on every Settings save (which would have pinned the old default forever). NOTE: accounts that saved Settings before this fix still have those two rows pinned in the DB, only new accounts (or rows cleared manually) get the key-aware default.
- [x] **[P2]** OpenRouter TTS and transcription options are broken as currently wired (live-tested 2026-07-02, corrected after reading OpenRouter's audio docs, the endpoints DO exist). **RESOLVED same day:** the TTS toggle now routes **Kokoro** voices via DeepInfra (default) or OpenRouter (new `kokoro_tts_provider` setting, OpenAI voices always use the OpenAI key), and the OpenRouter transcription option was removed (old saved values fall back to DeepInfra/OpenAI on load). Original findings kept below for reference. (a) **TTS**: `/api/v1/audio/speech` is OpenAI-compatible and works (verified: `hexgrad/kokoro-82m` + voice `af_bella` returned a real MP3), but OpenRouter carries **no OpenAI TTS models**, `openai/gpt-4o-mini-tts` does not exist there. Audio models are hidden from the default catalog, list them with `/api/v1/models?output_modalities=speech`. So the "OpenAI voices via OpenRouter" toggle can never work as designed. Either remove the toggle or repurpose it: routing **Kokoro** voices via OpenRouter works today and needs only a small change in `getTTSClientForUser()` (`ai-providers.ts`). (b) **Transcription**: `/api/v1/audio/transcriptions` is NOT OpenAI-SDK compatible. It requires a JSON body (`{model, input_audio: {data: <base64>, format}}`) and rejects the SDK's multipart upload, so the current OpenRouter path in `getTranscriptionClientForUser()` always fails. Worse, the endpoint returns plain text only, **no word timestamps and no segments** (its only params are language/temperature/provider), so even a custom call could never power read-along. Recommend removing the OpenRouter transcription option, or keeping it as text-only with a clear warning.
- [x] **[P3]** Settings placeholder hints suggest `google/gemini-3-flash` for OpenRouter, but that model ID does not exist there. The valid ID is `google/gemini-3-flash-preview` (verified working via a live vision test, 2026-07-02). Fixed same day: both hints in `SettingsPage.tsx` (chat-provider hint and image-description placeholder) now say `gemini-3-flash-preview`.
...
- [ ] **[P4]** Strip MathJax `<style>` from EA Forum/LessWrong (turndown/editor path only). Math articles embed a huge MathJax CommonHTML stylesheet (`.mjx-chtml {...}` wall) right next to the equation, which dumps into the Markdown editor. **Strip it in the turndown HTML→Markdown converter only (a `remove(['style','script'])`), NOT at fetch**. That stylesheet is *load-bearing for display*: MathJax renders math as plain `<span>`s positioned entirely by this CSS (superscripts, fractions, fonts), so removing it from `html_content` would break how math looks in the Content tab. Editor can't render the math anyway (it's just text there), so stripping it there is pure cleanup. Separate/bigger: render math ourselves (KaTeX from the LaTeX source) so it looks right everywhere AND survives editing. Currently it degrades to plain text (`xa` instead of `xᵃ`).
...
- [ ] **[P2]** **Run `VACUUM FULL content_items;` in DbGate** to physically shrink the database file (~2 GB → a few hundred MB) now that the audio blobs are cleared. CAUTION: locks the table for the duration (likely 1-3 min), so the app will pause. Do it when not listening. Optional (disk is cheap) but makes the `📦 [Storage]` table-total number honest again.
- [ ] **[P3]** Set `JWT_SECRET` in the backend's Railway variables (any long random string). Startup logs warn it's unset, which means **everyone gets logged out on every deploy/restart**. That's why the app keeps asking to log in. One-time fix.
- [ ] **[P3]** Optional cleanup: the `CLEAR_AUDIO_BLOBS=true` variable can be removed now (harmless if left, each boot it just finds 0 blobs to clear).
...
- [x] **[P4]** Removed backend/src/services/content-alignment.ts, which implemented Needleman-Wunsch sequence alignment via the `seqalign` package, exactly the algorithmic alignment approach this project's CLAUDE.md explicitly bans ("NEVER Use Fuzzy Matching or Algorithmic Alignment", tried before and always failed). It wasn't imported anywhere else. Deleted the file, backend/src/types/seqalign.d.ts, and the seqalign dependency from backend/package.json (2026-07-01)
...
- [x] **[P3]** Read-along: split `<ul>`/`<ol>` into per-`<li>` elements so lists highlight item-by-item instead of the whole block at once. Done in `llm-alignment.ts` `extractContentElements()` (each item re-wrapped in its list type, `<ol>` gets `start` so numbering stays correct, only shapes the alignment/read-along data, stored html_content untouched). Takes effect on next alignment regen. (2026-06-14)
  - [ ] Still one chunk: a sub-list nested inside an item (deferred), and the footnote section `<ol>` (could split the same way). Inter-item spacing not tuned yet.
...
- [x] **Defaults tuning + never-pin behavior + key UI fix** (2026-07-02, follow-up): (1) the key remove button now sits to the RIGHT of the input field (it was stacking on top of the eye toggle, both were absolutely positioned at the same spot). (2) **Blank model fields are no longer pinned**: the form used to pre-fill defaults into the fields and persist them on Save, freezing that day's defaults into user settings forever. Model fields now stay blank unless explicitly set, and blank = the live default (placeholders advertise it), matching the custom-prompts never-pin principle. (3) DeepInfra chat default switched from DeepSeek V3.2 (~20 tok/s serving, disliked) to **openai/gpt-oss-120b** (~46 tok/s, cheaper, supports reasoning effort, validated live). (4) OpenRouter chat default is now **openai/gpt-5-mini** (OpenRouter carries OpenAI chat models, only their TTS models are absent). (5) **Anthropic jobs default reasoning effort to high** (Haiku degrades without thinking, `CHAT_DEFAULT_EFFORT`, the OpenAI-compat endpoint accepts reasoning_effort, validated live). (6) DeepInfra's provider config now passes reasoning_effort through (was silently dropped).

- [x] **Key-aware defaults for every feature** (2026-07-02): with nothing configured, each feature now picks a working default from whichever API keys exist. Chat features (narration/alignment/summaries): first key in the order OpenAI (**gpt-5-mini**, replacing the old gpt-5-nano legacy fallback) → DeepInfra (DeepSeek V3.2) → Anthropic (claude-haiku-4-5) → Gemini (gemini-3-flash-preview) → OpenRouter (anthropic/claude-haiku-4-5). TTS: **Kokoro Puck** when DeepInfra or OpenRouter exists, else OpenAI coral. Transcription (already key-aware): DeepInfra else OpenAI. Image descriptions: Gemini → DeepInfra → OpenAI → OpenRouter (was hardcoded Gemini, which broke the feature for accounts without a Gemini key). Reasoning effort: blank = the model's own default everywhere. Frontend pre-fills mirror the same logic (`CHAT_MODEL_DEFAULTS`, `defaultImageProvider`).

- [x] **API key remove buttons + OpenAI image descriptions** (2026-07-02): every configured API key now has a trash button next to the eye toggle (stores `''`, which the backend already treats as unset everywhere). Added `openai` as an image-description provider (`describeViaOpenAI()` in `image-alt-text.ts`, default `gpt-5-mini`, vision verified live, no temperature param since the GPT-5 family rejects non-default values), so an OpenAI key alone also unlocks every feature. Shortened the API-keys intro to the one-line recommendation: GPT-5 Mini (OpenAI) for narration and read-along, DeepInfra for transcription and TTS.

- [x] **Settings polish pass with user-authored copy** (2026-07-02): user rewrote the Settings descriptions by hand, Claude implemented the inline comment requests: (1) Account section now shows a **feature-availability panel** (green check per feature, red X + required key when missing, scroll-link to API keys, single green "All features available" when complete), (2) chat/transcription/image model hints now read `default = ...` and a **blank model field genuinely uses the per-provider default** (new `CHAT_DEFAULT_MODELS` in `ai-providers.ts`, OpenAI default = gpt-5-mini), (3) Wallabag "How to connect" instructions are collapsed behind a chevron, (4) Wallabag buttons restyled to match the app (the base class had no CSS at all, they were raw browser buttons), (5) fixed the Kokoro voice-chip layout (the generic `.form-group label` rule outranked `.voice-chip`, turning chips into block labels with wrapped text, fixed via `.settings-section .voice-chip` + nowrap), (6) "jobs" are now called "features" in UI text, (7) fixed a JSX syntax error in the user's TTS-voices label edit ("(N selected)" count).

- [x] **DeepInfra image descriptions + recommended-key reorder** (2026-07-02): added `deepinfra` as an image-description provider (`describeViaDeepInfra()` in `image-alt-text.ts`, OpenAI-compatible vision, default model `google/gemma-4-26B-A4B-it`). Live test: Gemma 4 26B described a test photo at the same quality as the Gemini 3 Flash baseline for ~1/20th the cost ($0.00003 vs $0.0006 per image). DeepInfra now covers every job in the app, so the Settings API-keys list puts it first and marks it recommended. Also made the image-description model field provider-aware (per-provider defaults, auto-swap on provider change so a Gemini model id never gets sent to DeepInfra, which also fixed the latent bug where the OpenRouter path defaulted to the invalid bare `gemini-3-flash-preview` id).

- [x] **Provider keys smoke-tested end to end** (2026-07-02): tiny live calls confirmed the OpenAI key works (gpt-5-nano), the Anthropic key works via Anthropic's OpenAI-compatible endpoint (claude-haiku-4-5, the same path the app's chat jobs use), OpenRouter chat works (anthropic/claude-haiku-4-5), and OpenRouter vision image descriptions work (google/gemini-3-flash-preview correctly described a test image). OpenRouter TTS also works with its own catalog (verified `hexgrad/kokoro-82m` returned a real MP3) and OpenRouter STT works via its JSON base64 format (verified `openai/whisper-1`), but the app's current wiring for both is broken and OpenRouter STT returns no word timestamps at all, details under Bug Fixes. DeepInfra and Gemini keys not exercised yet. Total test spend under $0.002.
...
- [x] **Prompt editor polish + em-dash cleanup** (2026-06-27): removed em dashes from all default prompts, replacing them with proper commas/periods (not a blind find-replace, each split into clean sentences) so the prompts practice what they preach ("Do not use em dashes"). Also de-em-dashed the user-facing Summaries section blurb. Prompt-editor UI: the "(customized, reset to default)" tag in the label is now the clickable reset (no separate button, nothing shows when a prompt is at its default), and there's padding between each prompt's description and its textarea.

- [x] **Editable prompts for the WHOLE app (central prompt registry)** (2026-06-27): every LLM prompt in the repo is now user-editable from Settings, not just summaries. New `prompt-resolver.ts` (`resolveCustomPrompt`/`fillPrompt`) + `prompt-registry.ts` (10 prompts with category/label/description/placeholder/warning metadata, served at `GET /api/users/prompts`). Each service keeps its default as an exported const (verified byte-identical to the old inline strings, so zero behavior change unless overridden) and resolves via `prompt_<id>` overrides. **The 10 prompts**: 6 summary (article/comment/podcast × single/multi, single-paragraph variants are now editable too), 2 narration scriptwriter (main + image-drop retry; `userId` threaded into `scriptArticleForListening`), 1 read-along alignment rules block, 1 image description. Settings → new top-level **"Custom prompts (advanced)"** section with one collapsible per category, "(customized)" tags, and reset buttons. Replaces the summaries-only version from earlier today.

- [x] **OpenRouter for everything + library card colour pass** (2026-06-21): expanded OpenRouter beyond chat so a single OpenRouter key can drive the whole app. **Transcription** via OpenRouter's OpenAI-compatible `/audio/transcriptions` (added as a 3rd `transcription_provider`). **TTS** via OpenRouter's `/audio/speech`. New `openai_tts_provider` (`openai` | `openrouter`) toggle routes the OpenAI voices through either key (same voices, the model id is namespaced to `openai/gpt-4o-mini-tts` for OpenRouter). `getTTSClientForUser` now returns `{ client, model }`; `pickRandomTTSVoice` + the Settings voice list treat OpenAI voices as available when routed via OpenRouter. **Image descriptions** via OpenRouter's OpenAI-compatible vision (`chat.completions` + base64 `image_url`). New `image_alt_text_provider` (`gemini` | `openrouter`) toggle; Gemini native SDK stays the default (only tested with Gemini Flash 3 on OpenRouter). Kokoro TTS still needs DeepInfra (not on OpenRouter). OpenRouter moved to the top of the API-keys list and its use-case line now lists all six jobs. **Library card colours**: content type is now a filled pill (Article=blue, Text=green, Podcast=purple, PDF=neutral) so it reads as strongly as the capability badges; the **Summary** badge took the orange that "Summarizing…" used; "Summarizing…" moved out of the badges into a blue progress banner (consistent with audio/transcript in-progress). **Restart consistency**: an interrupted summary now resets to `failed` (with a Retry banner) instead of silently reverting to idle.

- [x] **Settings UI redesign + error-banner fix** (2026-06-21): reworked `SettingsPage.tsx` into a cohesive layout. **Section order** is now Account → Audio generation → Summaries → Playback → API keys → **Models** (new) → Wallabag sync. **Audio generation** is trimmed to just the "runs automatically" toggles (auto-generate audio, auto-transcribe podcasts, comment-narration toggles + cutoff). All provider/model pickers moved into a dedicated **Models** section: Narration / Read-along alignment / Summaries / Transcription / **TTS voices** (own card; voice groups titled `Model name (Provider)`, e.g. `gpt-4o-mini-tts (OpenAI)`, `Kokoro-82M (DeepInfra)`) / **Image descriptions** (own card, enable checkbox inside). Fixed the "Use the same model as Narration" checkbox (was stretching full-width and pushing its label right, now left-aligned via `.checkbox-inline`). **API key descriptions** rewritten as terse two-liners: line 1 = the jobs that key powers (narration/read-along/summaries/TTS/transcription/image descriptions), line 2 = a "Get a key" link (OpenRouter also links to its **Compare** pricing page). Standardised every description to one muted colour (`.settings-hint`/`.section-description` → `var(--t3)`), dropping the old `#666`/`#888`/blue mix. Also redesigned the library card **error banner** (`ContentCard.tsx`): message on the left, Retry + dismiss-X grouped together on the right at matching heights (was a staggered absolute-positioned X with the Retry button wedged under the text), applies to both generation and summary failures.

- [x] **Multi-provider model selection overhaul + reasoning effort** (2026-06-21): added a provider registry (`CHAT_PROVIDERS` in `ai-providers.ts`) covering **OpenAI, DeepInfra, OpenRouter, Anthropic, Google Gemini**, all via the OpenAI SDK (different baseURL+key). The single shared "Narration LLM" became **per-job model selection**: `getChatClientForJob(userId, 'narration'|'alignment'|'summary')`, each with its own provider/model/`reasoning_effort` (empty = provider default, so no behavior change unless set). Read-along & summaries default to "use same model as narration" (checkbox). Transcription got explicit provider (DeepInfra/OpenAI) + model; image descriptions stay Gemini-only but the model is now a free-text setting. Settings UI reorganized into cohesive "AI job" cards (`renderChatJob()`), with OpenRouter + Anthropic key fields and per-provider placeholder hints. **Read-time fallback** from the legacy `narration_llm` means existing users keep working and the fields pre-fill with their current model, nothing wiped. New settings keys allowed/masked in `users.ts`.

- [x] **Fix summary failures ("Premature close") + surface them in-app** (2026-06-21): bulk summaries were all failing with `Invalid response body ... Premature close`. Root cause is Node #63989 (a Node security release made reused keep-alive sockets throw `ERR_STREAM_PREMATURE_CLOSE`), hitting the old `openai` SDK's `node-fetch@2` pooling; the floating `node:22-slim` tag pulled the patched Node on a rebuild (no code change). TTS/transcription survived because they retry; summaries didn't. Fixes: (1) `chatCreateWithRetry` in `summarizer.ts`, exponential backoff on connection/429/5xx; (2) new `summary_error` column (migration 021) + a "Summary failed: … [Retry]" display on library cards (`ContentCard.tsx`) since summary failures were never rendered (only `generation_status==='failed'` was); (3) truncate feed `guid`/`title` to 500 before insert (`podcast-service.ts`). This fixes `value too long for varying(500)` dropping one feed item. ROOT-CAUSE FIX DONE: upgraded `openai` 4.x → **6.44** (zero-dependency, native `fetch`/undici, removes `node-fetch@2` and the legacy `http.Agent` pooling entirely). Backend builds clean; all usage (constructor, chat/speech/transcription create) type-checks unchanged. **Needs a runtime smoke-test on deploy** (generate audio = TTS, transcribe a podcast = Whisper file upload, generate a summary) since those binary/file paths can't be verified offline.

- [x] **Markdown editing for articles & texts + version history** (2026-06-14): full-fidelity HTML↔Markdown conversion in `frontend/src/markdown.ts` (turndown + turndown-plugin-gfm for HTML→MD, marked for MD→HTML). Custom rules round-trip LessWrong/EA LLM blocks (`> [!ai]` Obsidian callout) and tweet embeds (`> [!tweet]`); GFM tables, standard `![alt](url)` images, native links/bold/italic; unrecognized HTML kept as raw islands (no data loss). Fullscreen player re-splits **Content** (editable, with a Markdown textarea + Write/Preview toggle) and **Read-along** (synced, read-only) tabs, and adds a **History** tab (view/restore snapshots). Add Text form gets a **Markdown/HTML** toggle. "Copy content" now preserves images/links/formatting (same converter as the editor). Editing is treated like a fresh fetch (audio/read-along left outdated until regenerated). Backend: migration `020_add_content_versions.sql`, PATCH `is_edit` path, snapshots before edit/refetch/restore, new `/:id/versions*` endpoints. Deps added: `turndown`, `turndown-plugin-gfm`, `marked` (all MIT). README now lists all dependency licenses.

- [x] **AUDIO-TO-VOLUME MIGRATION COMPLETED IN PRODUCTION** (2026-06-12): all 44 audio files (851 MB) verified on the `/data` volume, playback + seeking confirmed by user, then `CLEAR_AUDIO_BLOBS=true` cleared all 44 DB blobs (`kept 0`). Postgres no longer stores or caches any generated audio. Remaining housekeeping tracked above (VACUUM FULL, JWT_SECRET, drop the env var).

- [x] **Fix volume never being detected (`require` in an ESM build)** (2026-06-12): `config/storage.ts` checked for the Railway volume with `require('fs').existsSync('/data')`. The backend is ESM (`"type": "module"`), where `require` is undefined, so the check always threw and silently fell back to ephemeral `./public`. Latent for ages (audio lived in the DB, so volume detection was never exercised). Now imports `existsSync` properly and detects via `RAILWAY_VOLUME_MOUNT_PATH` then `/data`. THIS is why the first migration boots logged `Using local storage` / `⚠️ NO VOLUME` despite the volume mounting.
- [x] **Audio-to-volume storage (code)** (2026-06-12): generated article/text audio now writes to `/data/audio/{id}.mp3` on the Railway volume instead of a Postgres `audio_data` BYTEA blob (the gold "Postgres" RAM line held 1-3 GB for days; RAM is ~$10/GB/mo vs. disk ~$0.15-0.25/GB/mo). New `services/audio-storage.ts` (save/size/delete/stream + migrate/clear helpers). Serving in `index.ts` streams the disk file first, falls back to the legacy DB blob (backward compatible). Generation falls back to storing the blob if the disk write fails (audio never lost). Archive/remove/delete paths now also delete the disk file (and the single-archive gate no longer depends on `audio_data` being present). Existing blobs auto-copy to disk at startup; `CLEAR_AUDIO_BLOBS=true` does the destructive blob-clear. Startup also logs `🧠 [Postgres]` memory settings. **Verification + clear still pending (see Performance §).**
- [x] **Backend starts with `node` directly instead of `npm start`** (2026-06-12): npm wrapped the process and logged a scary-but-harmless `npm error signal SIGTERM` on every normal shutdown/redeploy, and could interfere with graceful shutdown. Dockerfile CMD is now `node dist/index.js`.
- [x] **Removed dead code: `GET /podcasts/:id/episodes` + `contentAPI.getEpisodes`** (2026-06-12): no callers anywhere in the frontend (user-approved removal).

- [x] **Storage breakdown in Railway logs on every startup** (2026-06-12): `index.ts` logs a `📦 [Storage]` block after DB init. Audio blob total (the number that matters for the 5GB volume), transcripts, word timestamps, HTML, comments, table + whole-DB size. Uses `pg_column_size()`/`pg_total_relation_size()` so nothing heavy is loaded. Fire-and-forget with try/catch, can never block or crash startup.
- [x] **Remaining `SELECT *` on content_items fixed** (2026-06-12): `wallabag-sync.ts` push loaded EVERY un-synced item's `audio_data` blob (10-50MB each) just to push text to Wallabag, now an explicit column list. `podcasts.ts` `GET /:id/episodes` shipped full transcripts + blobs for all episodes, now mirrors the lean `GET /api/content` column list (endpoint has no frontend callers; flagged as dead code).
- [x] **Stop loading the old audio blob on every (re)generation** (2026-06-12): `generateAudioForContent()` opened with `SELECT * FROM content_items`, which drags `audio_data` (the 10-50MB BYTEA audio blob) plus `transcript_words` / `content_alignment` into backend memory at the start of every audio generation. On a *regenerate* that's tens of MB loaded only to be overwritten seconds later (and pulled into Postgres's RAM cache, where it lingers for days, see the memory-usage investigation). Replaced with an explicit column list of only the fields the function reads. No behavior change.
- [x] **Fix frontend build break in batch-poll PR** (2026-06-12): `getStatuses()` declared `generation_progress`/`error`/`current_operation` as `number|null` / `string|null`, but `ContentItem` types them as optional (`number|undefined`). Passing the status object to `updateItem(Partial<ContentItem>)` failed `tsc -b`, which broke the Railway frontend deploy. Now references the `ContentItem` field types directly.
- [x] **Batch generation-status poll (stop downloading full items every 2s)** (2026-06-11): while any item was generating, the library polled `GET /content/:id` every 2 seconds, which returns the ENTIRE item (full transcript, 9,000+ word timestamps, alignment, comments), ~0.5 MB per transcribed podcast. Bulk-summarizing 10 hour-long episodes downloaded on the order of 150 MB/min, the same class of bug as the 80 GB incident. Fixed with a lean `POST /content/status` endpoint that returns only the status fields (`generation_status`, `generation_progress`, `current_operation`, `generation_error`, `summary_status`) for a batch of ids in ONE request (a few hundred bytes/tick). The full item is still fetched once, at completion, via `refreshItem`. The poll now also correctly refreshes on summary completion (not just audio). Frontend: `contentAPI.getStatuses()` + rewired poll in `LibraryTab.tsx`.
- [x] **Fix chunked podcast transcription failing at chunk 2 (DeepInfra 400)** (2026-06-11): for files >25 MB, chunk 2+ sent the ENTIRE previous chunk's transcript (>10k chars) as the Whisper prompt whenever the initial prompt was empty, which it always is since `buildWhisperPrompt()` returns ''. OpenAI silently truncates long prompts; DeepInfra rejects with a bare 400. The continuation prompt is now always bounded (last 200 chars). Short, unchunked episodes were never affected.
- [x] **"Generate All Summaries" (user menu) now includes podcasts** (2026-06-11): it silently skipped podcasts, the transcript-first warning was only wired into the Select-mode bulk path. The user-menu button now includes podcasts and shows the same modal (generate transcripts + summaries / skip episodes without transcript / cancel).
- [x] **Tweet-mode [N more] expands inline** (2026-06-11): clicking [N more] on a library card now reveals the remaining summary paragraphs on the card itself (article summary only, never the comment summary) instead of jumping to the fullscreen Summary tab.
- [x] **Mini player is audio-only + latent bug fixes + lint cleanup** (2026-06-11): items without audio open fullscreen only (no mini player, no minimize button, close is the way out); fixed the MiniPlayer rules-of-hooks landmine (early return before two useMemo hooks); fixed library cards always saying "Generate transcript" for podcasts that have one (checked the `transcript` column which the list endpoint never sends, now uses `transcript_words` like the badge); lint 62 → 44 errors (annotated empty catches, dropped unused catch bindings; remaining are `no-explicit-any` style issues plus one false-positive).
- [x] **Podcast summaries** (2026-06-11): see the marked task under Features, transcript-based summaries with the transcript-first warning modal (single + bulk). The RSS episode description is passed as context (correct guest-name spellings; the prompt notes the transcript may contain transcription mistakes and to trust the description on name conflicts).
...
- [x] **"Copy content" in fullscreen player dropdown** (2026-06-11): copies title • author • date • link, the article body as Markdown, and the nested comment thread to the clipboard. This is what Ctrl+A/Ctrl+C should have produced without the player chrome.
- [x] **Fix shuffle preference lost on reload** (2026-06-11): saved `queue_shuffle` was only applied at startup if the library had already finished loading, a race that usually dropped the preference (autoplay had no such dependency, which is why it persisted reliably). The flag is now set unconditionally and the shuffle order is built lazily on first use, rotated so the clicked item sits at position 0.
- [x] **Fix queue jumping backwards after archiving the playing item** (2026-06-11): "Up next" pivoted on the current item's position in the *filtered* list; archiving the playing item (which also wipes its audio) removed it from that list, so the stream fell back to the original library click and replayed old items. The pivot is now position-based on the full id stream (shuffle order or library order), so it survives the playing item being archived mid-track and continues forward. Also unified the duplicated ordering logic in `getNonManualItems`/`getPrevItem` into one `getStream()` helper; Prev now steps back to the nearest still-playable item.
- [x] **Library search** (2026-06-11): Search icon in the library filter row expands into a full-width search bar (debounced, client-side). Searches title, author, description, tags, podcast show name AND the full article/text body. Cards matching only in the body show a "matched in text: …" snippet. The queue's "Up next" context now includes the search query.
- [x] **Two-dimension library filters (type × status)** (2026-06-11): The single filter row is now type chips (All/Articles/Texts/Podcasts) + a funnel button opening a status menu (Active/Favorites/Archived), so "archived podcasts" or "favorite texts" finally work. The funnel button lights up and shows the status name when not on Active. Semantics: Active = not archived, Favorites = starred (incl. archived), Archived = archived.
- [x] **Bulk actions via `POST /api/content/bulk`** (2026-06-11): Select mode's bulk bar now does star/archive-or-unarchive/delete in ONE request (previously one request per item) plus an overflow (⋮) menu: unstar, unarchive, remove audio, remove summaries, and sequential generate audio / generate summaries / refetch from web (with cost-confirmation dialogs and a progress counter). Bulk archive mirrors single-item archive (wipes audio for non-starred articles/texts); bulk unarchive deliberately does NOT auto-regenerate audio. Bulk delete propagates to Wallabag.
- [x] **Removed Settings "Reset generated data" section** (2026-06-11): superseded by Select All + Remove audio/Remove summaries under the right filter. Backend `wipe-all-audio`/`wipe-all-summaries` endpoints removed too.
- [x] **Library "Twitter-feed" mode** (2026-06-08): `library_show_summary` setting (Summaries section) shows the full article summary on library cards in place of the description (comment summaries excluded; falls back to description when no summary). List endpoint now returns `summary`.
- [x] **Summary prompt tweaks** (2026-06-08): all summarizer prompts now ban em dashes/hyphens as sentence breaks and prefer several short simple sentences per paragraph; the article multi-paragraph prompt also says to highlight only the most interesting stories when the article is a news roundup.
- [x] **Multiple TTS voices with random rotation** (2026-06-08): Settings → Audio Generation has a "Voices" multi-select (OpenAI + full Kokoro voice list, American/British male & female, labelled AF/AM/BF/BM, can mix providers). Only providers whose API key is set are shown. When one or more are ticked, each new audio generation picks a random voice (the picked voice's model also routes the TTS client, so cross-provider rotation works); empty = single voice. Stored as `tts_voices` (JSON `{ model, voice }[]`); `pickRandomTTSVoice()` in `ai-providers.ts` (skips voices whose provider key is missing), applied in `generateArticleAudio()`.
- [x] **Article + comment summaries (Twitter-thread style)** (2026-06-08): New "Summary" tab in the fullscreen player (appears once a summary exists, immediately right of the Content tab). Generated by the narration LLM (`backend/src/services/summarizer.ts`) from the player dropdown, the library dropdown, or automatically when an article is added (`auto_generate_summary` setting). Two separate summaries: article body and (optional) comment discussion, rendered with an orange divider between them; the comment summarizer receives the article as context only. Each paragraph reads like a tweet (≤280 chars). **Length is computed in code**: article/comment character count → matching tier in the editable `summary_tiers` setting → `maxTweets` injected into the prompt (the model never counts characters). An independent `summary_status` column lets audio and summary generation run at the same time (library/player show a "Summarizing…" indicator). A "Generate All Summaries" action in the user dropdown (under "Generate All Audio") bulk-generates summaries for all non-archived articles/texts that don't have one yet. New endpoint `POST /content/:id/generate-summary`; migration `019_add_summary_columns.sql`; new settings: `auto_generate_summary`, `summarize_comments`, `summary_tiers`.
- [x] **Episode search within podcast/RSS feeds** (2026-05-19): Search button next to episode/preview section headers. Searches the full RSS feed server-side (not just the 50 loaded episodes) so users can find old episodes. Shows instant client-side results while server-side search is in flight, with loading indicator. Backend: `searchFeedEpisodes()` + `GET /podcasts/search-feed` endpoint. Frontend: debounced 400ms search in FeedTab.tsx.
- [x] **Server-side Feed Tab pagination** (2026-05-19): All Feed tab views (Recent Updates, podcast detail, search result preview) now use proper server-side pagination with 50-item batches. Recent Updates paginates from database with OFFSET. Podcast/search detail paginates from server-side XML cache (RSS XML downloaded once, parsed incrementally with offset/limit). Replaced old client-side fake pagination that had a hard 100-item cap. XML cache: 5-min TTL, max 20 feeds, iterative regex parsing avoids OOM on huge feeds (e.g. JRE 2500+ episodes).
...
- [x] **Fix four compounding playback bugs** (2026-04-17):
  ...
  - **Headphone removal resuming paused audio**: `handlePause` only set `userPausedRef` for app-UI pauses, not hardware/OS pauses. Now ALL pause events set `userPausedRef=true`, so rogue play events from headphone removal or Bluetooth re-routing are always blocked. Only app-UI play (togglePlay) or queue auto-play can resume.
  - **Play/pause inconsistency**: Side effect of the above bugs, fixed by the same changes.
...
- [x] **Fix un-archive auto-generating audio** (2026-04-04): Un-archiving was always triggering audio generation regardless of user setting. Now checks `auto_generate_audio_for_articles` setting first.
- [x] **Optimize library filter switching to instant client-side filtering** (2026-03-29): Filter buttons (Articles, Texts, Podcasts, etc.) no longer make an API call per click. All items are fetched once on mount, stored in a master list (`allItems`), and filtered client-side. Switching filters is now instant.
- [x] **Substack comments: "upvotes" → "likes"** (2026-03-29): Substack uses "likes" not "upvotes". Updated narration (openai-tts.ts), LLM alignment (llm-alignment.ts), and frontend display (FullscreenPlayer.tsx) to show "N likes" for Substack content and "N upvotes" for EA Forum/LessWrong.
...
- [x] **Configurable max comment narration limit** (2026-03-28): Users can set max comments to narrate in Settings (default 50). Auto-generate and bulk generate skip articles exceeding this limit. Manual generation shows a warning but still allows proceeding.
- [x] **Bulk generate skips high-comment articles** (2026-03-27): "Generate All Audio" skips articles exceeding the user's max comment limit and shows summary of skipped articles.
...
- [x] **Add comment narration settings (EA Forum, Substack)** (2026-03-27): Two toggles in SettingsPage for EA Forum/LessWrong and Substack. When disabled, comments still display in read-along but without audio sync.
...
- [x] **RSS/Atom Feed Improvements** (2026-02-01):
  ...
  - Fixed HTML entity decoding in descriptions (&#8217; → ', &#163; → £, &#8212; → dash)
...
- [x] Fix EA Forum/LessWrong comment reactions display with dynamic extendedScore support for all reaction types (agree, disagree, love, etc.) (2026-01-25)
...
- Investigate Gemini native TTS as a voice provider. The Gemini API has TTS models, but they need the native SDK and return raw PCM, so it's a new client path plus a new voice catalog. Note: Gemini TRANSCRIPTION is not viable for read-along (no word timestamps, same limitation as OpenRouter), so only TTS is worth exploring.
