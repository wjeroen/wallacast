...
- [ ] **[P1]** **Audio-to-volume migration — FINISH IT** (code shipped 2026-06-12, awaiting verification). Generated audio now writes to `/data/audio/{id}.mp3`; serving prefers the disk file and falls back to the DB blob; existing blobs auto-copy to disk at startup (non-destructive, logged `🎵 [AudioMigration]`). REMAINING STEPS FOR USER: (1) deploy + confirm audio still plays (a few articles, with seeking); (2) set `CLEAR_AUDIO_BLOBS=true` in Railway to NULL the in-DB blobs (frees ~851 MB of RAM-cached data); (3) optional: run `VACUUM FULL content_items` in DbGate to return the dead disk space to the OS. Watch the `📦 [Storage]` + gold Postgres line in the memory graph drop afterward.
