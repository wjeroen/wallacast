...
- [ ] **[P1]** **Audio-to-volume migration — FINISH IT** (code shipped 2026-06-12, awaiting verification). Generated audio now writes to `/data/audio/{id}.mp3`; serving prefers the disk file and falls back to the DB blob; existing blobs auto-copy to disk at startup (non-destructive, logged `🎵 [AudioMigration]`). First deploy's logs showed `Using local storage: /app/public` — i.e. the container did NOT see the volume at `/data` (files went to ephemeral disk that boot). A hard safety guard now refuses `CLEAR_AUDIO_BLOBS` unless the volume is detected, and startup logs ✅/⚠️ which storage is live. REMAINING STEPS FOR USER: (1) check current boot logs for `✅ Persistent volume detected at /data` (if ⚠️, fix the volume's mount path in Railway — must be exactly `/data`); (2) confirm audio plays (a few articles, with seeking); (3) set `CLEAR_AUDIO_BLOBS=true` to NULL the in-DB blobs (~851 MB); (4) optional `VACUUM FULL content_items` in DbGate. Watch the `📦 [Storage]` + gold Postgres line drop afterward.
