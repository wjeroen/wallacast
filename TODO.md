...
- [x] **Multiple TTS voices with random rotation** (2026-06-08): Settings → Audio Generation now has a "Voice variety" multi-select (OpenAI + Kokoro voices, can mix providers). When one or more are ticked, each new audio generation picks a random voice from the list (the picked voice's model also routes the TTS client, so cross-provider rotation works); empty = the single voice as before. Stored as `tts_voices` (JSON `{ model, voice }[]`); `pickRandomTTSVoice()` in `ai-providers.ts`, applied in `generateArticleAudio()`.
